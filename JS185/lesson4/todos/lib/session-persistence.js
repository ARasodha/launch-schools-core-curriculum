...
module.exports = class SessionPersistence {
  ...
  // Takes input todoListId and returns todoList with that id, if none exist returns undefined
  loadTodoList(todoListId) {
    let todoList = this._findTodoList(todoListId);
    return deepCopy(todoList);
  }

  // Return sorted todo list by completed todos and title (case-insensitive)
  ...
  // Fubd a todo with the indicated ID in the indicated todo list. Returns `undefined` if not
  ...
  // Toggle a todo between the done and not done state. Returns `true` on success. `false` 
  // if the todo or todo list doesn't exist. The id arguments must be numeric.
  ...
  // Returns a reference to the indicated todo list in the indicated todo list.
  // Returns a `undefined` if either the todo list or the todo is not found. Note
  ...
  // Deletes the specified todo. Returns `true` if todo item deleted, returns false if not.
  // Takes todo list todo item IDs which must both me numeric.
  ...
  // Mark all todos on the todo list as done. Return `true` on success, `false` if the todo list
  // doesn't exist. The todo list ID must be numeric.
  ...
  // Delete a todo list from the list of todo lists. Return `true` one a success,
  // `false` if the todo list doesn't exist. The ID argument must be metric.
  deleteTodoList(todoListId) {
    let todoList = this._findTodoList(todoListId);
    if (!todoList) return false;

    let todoListIndex = this._todoLists.indexOf(todoList);
